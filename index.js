...
const { menu } = require('./language/menu.js')
...
client.on('qr', qr => {
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color(' Scan This QR Code','white'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const balesCepet = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
         ...
         const isRegistered = checkRegisteredUser(sender)
			...
			const costum= ( pesan , tipe, target , caption) => {
			client.sendMessage(from, pesan , tipe , {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: {"imageMessage":{url: 'https://mmg.whatsapp.net/d/f/Ahj0ACnTjSHHm6-HjqAUBYiCu2-85zMZp_-EhiXlsd6A.enc',mimetype: 'image/jpeg',caption: `${caption}`,fileSha256: '0Pk0qJyQFn9FCtslZrydJHRQDKryjYcdP7I3CmRrHRs=',fileLength: '20696',height: 360,width: 382,mediaKey: 'N43d/3GY7GYQpgBymb9qFY5O9iNDXuBirXsNZk+X61I=',fileEncSha256: 'IdFM58vy8URV+IUmOqAY3OZsvCN6Px8gaJlRCElqhd4=',directPath: '/v/t62.7118-24/35174026_475909656741093_8174708112574209693_n.enc?oh=2a690b130cf8f912a9ca35f366558743&oe=6061F0C6',mediaKeyTimestamp: '1614240917',jpegThumbnail: '',scansSidecar: 'choizTOCOFXo21QcOR/IlCehTFztHGnB3xo4F4d/kwmxSJJIbMmvxg==',scanLengths: [Array],midQualityFileSha256: '68OHK4IyhiKDNgNAZ3SoXsngzYENebQkV4b/RwhhYIY=',midQualityFileEncSha256: '2EYOLCXx+aqg9RyP6xJYChQNbEjXZmc0EcSwHzoyXx0='}}}})
			}
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (bad.includes(balesCepet)) {
				...
				/*[-- function antilink --]*/
				if (balesCepet.includes("://chat.whatsapp.com/")){
					if (!isGroup) return
					if (!isAntiLink) return
					if (isGroupAdmins) return reply('karena kamu adalah admin group, bot tidak akan kick kamu')
					client.updatePresence(from, Presence.composing)
					if (balesCepet.includes("#izinadmin")) return reply("#izinadmin diterima")
		...
		if(balesCepet.includes('proses')){
		  if (!isOwner) return
	client.sendMessage(from, `
┏━━━━━━━⊱ *DETAIL TRANSAKSI* ⊰━━━━━━━┓
┃${a}STATUS : PROSESS${a}
┃${a}TIME   : ${JAM}${a}
┃${a}FROM   : DROP DATA HANS SHOP${a}
┃${a}DATE   : ${tanggal()}${a}
┗━━━━━━━⊱ *DETAIL TRANSAKSI* ⊰━━━━━━━┛
`, text, {quoted: mek})
	}
	if(balesCepet.includes('done')){
	  if (!isOwner) return
	client.sendMessage(from, `
┏━━━━━━━⊱ *DETAIL TRANSAKSI* ⊰━━━━━━━┓
┃${a}STATUS : SUKSES${a}
...
`, text, {quoted: mek})
			...
			switch(command) { 
				...
				if (args.length < 1) return reply('yang mau dijadiin text sticker apaan, titit kah?')
					...
					anu = await fetchJson(`https://mhankbarbar.tech/api/text2image?text=${teks}&apiKey=Bimbimesaputra15`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
					fs.unlinkSync(ranp)
					if (err) return reply(meas.error.stick)
     		...
     		await limitAdd(sender)
				...
				case 'moddroid':
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(`MAAF ${pushname} ANDA BUKAN USER PREMIUM`)
				if (isLimit(sender)) return reply(ind.limitend(pusname))
 ...
 if (!isRegistered) return reply(ind.noregis())
 if (isLimit(sender)) return reply(ind.limitend(pusname))
anu = await fetchJson(`https://ferdiz-api.herokuapp.com/api/stalkig/user/name=${args[0]}`, {method: 'get'})
...
client.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
               ...
               case 'help': 
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					await costum(menu(pushname, prefix, getLevelingLevel, getLevelingXp, sender, reqXp, _registered, uangku, role, premi, client, process, time), text, tescuk, cr)
					break 
				case 'groupmenu':
				await costum(ind.groupmenu(prefix), text, tescuk, cr)
				break 
				case 'funmenu':
				  await costum(ind.funmenu(prefix), text, tescuk, cr)
				  break 
				  case 'makermenu':
				 await costum(ind.makermenu(prefix), text, tescuk, cr)
				 break

				case 'vilogff':
				await costum(ind.vilogff(), text, tescuk, cr)
				break
				case 'ff':
				await costum(ind.ff(), text, tescuk, cr)
				break
				case 'ml':
				await costum (ind.ml(), text, tescuk, cr)
				break
				case 'payment':
				await costum (ind.payment(), text, tescuk, cr)
				break
				...
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
				await	costum(donasi(), text, tescuk, cr)
					break
				case 'admin':
         	   case 'owner':
         	   case 'creator':
				...
				case 'limit':
				   ...
				   checkLimit(sender)
					break 
				case 'giftlimit': 
				if (!isOwner,!isPrem) return reply(`MAAF ${pushname} ANDA BUKAN USER PREMIUM`)
